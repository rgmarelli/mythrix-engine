...
for an exit code, choosing human vs. JSON rendering — and returns a process
exit code. The Typer-decorated `query` command below is a thin wrapper that
builds real `KuzuGraphStore`/`ChromaVectorStore`/`OllamaEmbedder` instances
via `core/bootstrap.py::build_stores` and delegates. This split is what lets
...
from mythrix.core.query_service import execute_query
