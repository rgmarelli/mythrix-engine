"""The chat panel's structured working memory (master spec.md FR97) —
...
# Session-scoped fields (master spec.md FR97's Context object) whose change
# identifies a different query subject, not just a display filter — these,
# plus a changed `region_id`, are what trigger a thread reset (FR96).
# `source_id`, `interpretant`, and `min_score` are also session-scoped per
# FR97, but they refine which hotspots are *shown*, not which hotspot the
...
class AgentUiSelection(BaseModel):
    """What the browser sends, as-is, each turn (master spec.md FR97) — the
...
def detect_thread_reset(previous: AgentContext, incoming: AgentUiSelection) -> bool:
    ...
    hotspot/subject the stored context was scoped to (master spec.md FR96): a different
...
def apply_ui_selection(context: AgentContext, incoming: AgentUiSelection) -> AgentContext:
    ...
    a thread-scoped field (master spec.md's "not yet determined" case, FR98),
    and `locator` (the hotspot's human-readable citation, e.g. "Ecclesiasticus
    43:1-4", FR101) always describes the same hotspot `region_id` does, so it
    ...
    a `None` there means "the UI hasn't set this," not "clear it" (FR97:
...
def backfill_from_tool_results(context: AgentContext, new_messages: list) -> AgentContext:
    ...
    (ADR 0006). No involvement from the generation model in deciding what to
    backfill.
