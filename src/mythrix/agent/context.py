"""The chat panel's structured working memory (spec.md "Context object") —
...
# Session-scoped fields (spec.md Context object) whose change identifies a
# different query subject, not just a display filter — these, plus a changed
# `region_id`, are what trigger a thread reset (FR3). `source_id`,
# `interpretant`, and `min_score` are also session-scoped per spec.md, but
...
class AgentUiSelection(BaseModel):
    """What the browser sends, as-is, each turn (spec FR4) — the client never
    pre-clears or diffs this before sending."""
...
def detect_thread_reset(previous: AgentContext, incoming: AgentUiSelection) -> bool:
    ...
    hotspot/subject the stored context was scoped to (spec FR3): a different
