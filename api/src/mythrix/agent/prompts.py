"""The operator system prompt (master spec.md FR-AG-05, FR-AG-06, FR-AG-09) — distinct from
...
* Every tool result you use returns a JSON payload. Number the citable items it contains in the order they appear, starting at 1 within that item's category: each graph fact / citation from get_symbol is a "G" item (G1, G2, ...), each passage segment from query_symbol or fetch_segments is an "S" item (S1, S2, ...). If a turn involves more than one tool call, keep counting across all of them rather than restarting at 1.
