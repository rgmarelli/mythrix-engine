...
from mythrix.cli.commands.load_signs import run_load_signs
...
def test_dry_run_reports_the_planned_diff_without_writing(
    ...
    exit_code = run_load_signs(FIXTURES_ROOT, graph_store=None, dry_run=True, as_json=True)
...
def test_real_load_makes_the_sign_queryable_afterward(graph_store: KuzuGraphStore) -> None:
    exit_code = run_load_signs(FIXTURES_ROOT, graph_store=graph_store, dry_run=False, as_json=False)
...
def test_human_readable_summary_mentions_counts(
    ...
    run_load_signs(FIXTURES_ROOT, graph_store=graph_store, dry_run=False, as_json=False)
...
def test_invalid_directory_reports_error_and_writes_nothing(
    ...
    exit_code = run_load_signs(bad_dir, graph_store=graph_store, dry_run=False, as_json=False)
