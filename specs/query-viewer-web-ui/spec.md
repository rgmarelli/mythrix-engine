...
- A form to pick one semiotic system, one sign, and one tradition, restricted to combinations that have a manifestation.
...
- A UI for comparing multiple interpretive traditions of the same sign against each other.
...
- FR1: A web UI presents sign query results equivalent to `mythrix query --json`'s evidentiary content, without requiring the CLI or reading raw JSON.
- FR2: The UI presents a form to select one sign and one tradition, restricted to sign/tradition combinations that have a manifestation.
- FR3: Submitting the form displays graph facts, then per-concept candidate passages, then concept-pair convergence groups for that sign/tradition pair, each rendered as soon as it is available rather than only after the complete result is ready.
...
- FR16: For each intersemiotic interpretant shown in graph facts, the display includes the target sign's own intrinsic properties and any interpretants recorded for that correspondence, not only the correspondence claim itself.
- FR20: The form presents a semiotic-system selector that scopes which signs the sign selector offers.
...
- FR7: A backend HTTP API serves sign/tradition listings and query results as JSON, as a process independent of the CLI.
