# Region Augmentation (`/augment`)

A confirmation-gated chat command that reads every region the consumer is
currently displaying against a free-text focus, delivers each reading as it is
produced, and consolidates the readings into one cited answer.
...
- FR-AU-17: An augmented region's passage is its full contiguous ordinal range,
  read verbatim by structural coordinate. Every segment whose ordinal lies
...
- FR-AU-32: The plan turn emits one confirmation instruction carrying the
  ...
  by the consumer itself. A rejected `/augment` emits no instruction.
- FR-AU-33: Both commands are declared in the capabilities document like every
  other command. `/augment` is listed to users; `/augment-confirm` is not.
