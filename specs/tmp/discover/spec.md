...
([agnostic-query.md](../../interfaces/agnostic-query.md)), reads every retrieved
region ([ranking.md](../../retrieval/ranking.md)) against a free-text analysis
focus, and consolidates the readings into one cited answer. Served by the
[Conversational Agent](../../interfaces/agent.md)'s chat panel and handled
entirely in code, without the orchestration model's tool selection.

Final home: `specs/interfaces/discovery.md`, requirement prefix `FR-DS`.
...
([agnostic-query.md](../../interfaces/agnostic-query.md) Non-goals).
`/summarize` reads one region, chosen by the user's own selection
([agent.md](../../interfaces/agent.md) FR-AG-33).
...
- Any new result kind, instruction type, panel, or rendering surface. The report
  is markdown through the existing path (FR-AG-23).
...
- FR-DS-01: The system offers a `/discover` command taking two inputs in one
  ...
  interpretant term list. The term list uses the same syntax, the same
  `:exact`/`:filter` directive vocabulary, and the same validation messages as
  `/query` ([agnostic-query.md](../../interfaces/agnostic-query.md) FR-AQ-02,
  FR-AQ-03).
- FR-DS-02: A `/discover` command that supplies no quoted focus, an empty focus,
  or no terms is rejected with a message naming the accepted syntax, without
  invoking the generation model or retrieving anything.
...
- FR-DS-05: `/discover` does not run. It parses its two inputs, holds them in
  ...
  of regions a run will analyze, and the literal command that runs it.
...
- FR-DS-08: At most one discovery is outstanding per session. A new `/discover`
  replaces any outstanding one. A confirmed discovery is cleared. A thread reset
  clears it.
...
- FR-DS-10: The ad-hoc retrieval, per-region analysis, and consolidation
  operations are not selectable by the orchestration model. They exist only on
  the deterministic discovery path, so the agent can still neither run nor
  narrate an ad-hoc query of its own accord.
...
- FR-DS-11: The retrieval step is the existing ad-hoc interpretant query,
  ...
  other query (FR-AQ-15–FR-AQ-17). It executes within the turn; a discovery
  emits no instruction and hands nothing to a consumer to execute.
- FR-DS-12: The regions analyzed, and the order they are analyzed and reported
  in, are exactly what retrieval returned, truncated from the top of the
  ranking. No model participates in selecting, dropping, re-ordering or
  re-scoring a region ([retrieval.md](../../retrieval/retrieval.md) FR-RT-03).
- FR-DS-13: The number of regions a run analyzes is configurable and bounded by
  default. When retrieval returns more regions than the bound, the report states
  how many matched and how many were analyzed.
...
- FR-DS-16: Each analyzed region produces one finding from exactly one
  ...
  and to say so when the passage does not bear on the focus.
...
- FR-DS-19: Every step of a run is logged at INFO: the parsed focus and terms;
  the count retrieved and the count analyzed; each region's index, label,
  locator and elapsed time as its analysis starts and completes; the
...
- FR-DS-21: Each analyzed region carries a region marker `[R1]`, `[R2]`, …
  numbered in the order the regions were analyzed. The consolidation cites the
  regions supporting each claim by marker.
...
- FR-DS-24: A reply composed entirely by the backend, with no model-authored
  ...
  in a user-supplied focus or term cannot fail such a turn.
- FR-DS-25: A finding carries no citation markers. Marker-shaped text a
  ...
  report — the analysis invocation is given no marker vocabulary, so such text is
  noise rather than a citation. The consolidation's region markers are the
  report's only markers.

### Registration and history

- FR-DS-26: Both commands are declared in the capabilities document like every
  other command ([agent-capabilities.md](../../interfaces/agent-capabilities.md)
  FR-CAP-02, FR-CAP-04). `/discover` is listed to users; `/discover-confirm` is
  not.
...
- FR-DS-28: A run's history entry consists of the user's literal command, the
  ...
  carries.
- FR-DS-29: The retrieval step's tool result carries each analyzed region's
