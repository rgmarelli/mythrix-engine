...
- **FR-FC-03**: The fact-checking call receives only the reply's sentences (each identified by its position) and the evidence — never the reply's own text as something to reproduce. It returns a classification: for each sentence that states a factual claim, whether it is supported, and if so, which evidence item id(s) support it. A sentence with no factual claim is omitted from the classification.
- **FR-FC-04**: The fact-checking call's response is verified structurally, not by comparing reproduced text: it must be a well-formed classification (a valid response format, each entry naming a sentence that exists and a supported/not-supported value). An entry that fails this check is discarded individually; a response that cannot be read as a classification at all is discarded entirely and the reply falls back to FR-FC-06.
