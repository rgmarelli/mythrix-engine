...
- FR-RT-18: An `"exact"`-directive interpretant contributes no embeddable query at all — unlike a `"filter"`-directive token, which is embedded (paired with a concept's own query) and so participates in per-concept retrieval (FR-RT-07) and concept-pair convergence (FR-RT-08/09), an `"exact"`-directive interpretant participates in neither; it exists only in this region-matching path.
- FR-RT-19: An `"exact"`-directive interpretant's match (FR-RT-17) is membership-only, like a `"filter"`-directive interpretant's (FR-RT-15) — no similarity score, since there is no query embedding behind it. A match reached this way is reported distinctly from one reached via a `"filter"`-directive token, so a consumer can tell the two apart.
- FR-RT-20: A region matched by only one interpretant is eligible and rankable regardless of that interpretant's kind ([ranking.md](ranking.md) FR-RK-03) — including one reached only through a literal-containment match (`"exact"` or `"filter"`), with no `"concept"` match nearby at all.
