...
- FR-RT-17: An interpretant carrying a `query.directive: "exact"` annotation ([corpus.md](corpus.md) FR-CO-03) is searched exclusively through a literal-text-filtered query of its own value — unlike `"filter"` (FR-RT-09), it contributes no separate unrestricted plain query, so every retrieved hit under its value is a guaranteed literal match. Its `query.as_token` is optional; when omitted, the literal filter searches the interpretant's own `value`. This filtered query stands in as the interpretant's own concept for scoring and pair-candidate purposes (FR-RT-07, FR-RT-08).
- FR-RT-18: An `"exact"`-directive interpretant's literal filter is scoped to its own concept's query only — it never appears as a pair member alongside an unrelated concept the way a `"filter"`-directive token does (FR-RT-09).
...
- FR-RT-19: An `"exact"`-directive interpretant (FR-RT-17) matches segments the same way a `"filter"`-directive interpretant does (FR-RT-15) — literal containment of its token, on whole-word boundaries, contributing membership rather than a similarity score. A match reached this way is reported distinctly from one reached via a `"filter"`-directive token, so a consumer can tell the two apart.
- FR-RT-20: Within one region, an `"exact"`-directive interpretant that has both a `"concept"` match and its own literal-containment match for the same value is reported once, not twice — the `"concept"` match (a real score) is kept and the literal-containment match is dropped as redundant. The literal-containment match is reported on its own only when no `"concept"` match for that value survives elsewhere in the region.
