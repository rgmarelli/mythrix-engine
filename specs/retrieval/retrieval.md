...
- FR-RT-04: Generated text the conversational agent layer produces is checked for grounding after the answer is composed, not enforced while composing it: a dedicated fact-checking model call classifies each sentence of the answer as supported or unsupported against the turn's tool-derived evidence, and a grounding score computed in code from that classification is appended to the reply as a visible footer ([ADR-025](../architecture-decisions/adr-025-post-hoc-fact-checker-replaces-self-citation.md)). The check never blocks, rewrites, or rejects the reply. The `query` path produces no generated text, so this requirement governs the conversational agent layer ([agent.md](../interfaces/agent.md) FR-AG-06) only.
