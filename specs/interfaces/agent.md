...
- FR-AG-40: Each turn is sent with the consumer's currently displayed regions, as an ordered list of region identities carried separately from the context object ([augmentation.md](augmentation.md) FR-AU-12). It is turn-scoped: it is never stored in the context object, never persists between turns, and changing it never resets the thread.
