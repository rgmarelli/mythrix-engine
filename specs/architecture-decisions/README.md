...
| [ADR-014](adr-014-slug-as-agent-entity-identity.md) | The slug is the only entity identity across the agent boundary — context fields and tool-result identity keys carry slugs, display names travel in separate display keys, and every entity-valued tool argument resolves either form to a slug | Accepted; sign-resolution consequence narrowed by [ADR-024](adr-024-tiered-exact-before-fuzzy-sign-resolution.md) |
...
| [ADR-022](adr-022-tool-owned-opaque-grounding-ids.md) | Each citable tool result item carries its own opaque, independently-generated `grounding_id`, assigned by the tool at render time; `turn_service.py` validates markers against those ids directly instead of reconstructing a positional count | Accepted; extends [ADR-006](adr-006-conversational-agent-orchestration-boundary.md) |
| [ADR-023](adr-023-in-graph-citation-retry.md) | An invalid citation in the model's reply gets a bounded, in-graph corrective retry (naming the bad marker) before falling back to the old one-shot reject; scoped to the `agent`-driven conversational path only | Accepted; extends [ADR-006](adr-006-conversational-agent-orchestration-boundary.md), [ADR-022](adr-022-tool-owned-opaque-grounding-ids.md) |
| [ADR-024](adr-024-tiered-exact-before-fuzzy-sign-resolution.md) | Sign resolution tries exact slug, then exact display name, then substring match, in that order, each tier authoritative the moment it has any candidates; `get_sign`/`query_sign` accept an optional `semiotic_system` argument scoping every tier to one system | Accepted; narrows [ADR-014](adr-014-slug-as-agent-entity-identity.md) |
