...
- **Ranking** — regions scored by summed, specificity-weighted match strength (a from-scratch lexical-IDF scheme, deliberately not BM25 — see [ADR 0002](specs/architecture-decisions/0002-dense-plus-exact-token-no-bm25.md) and [ADR 0004](specs/architecture-decisions/0004-absolute-floor-and-lexical-specificity-ranking.md) for why).
- **Agent** — a bounded tool-calling loop over a fixed, read-only tool set; retrieval stays deterministic even when a model is in the orchestration loop ([ADR 0006](specs/architecture-decisions/0006-conversational-agent-orchestration-boundary.md)).
